...
async def content_process(state: SourceState) -> dict:
    try:
        ...
        if file_path:
            ...
            # 1. TXTファイルのバイパス処理
            # ---------------------------------------------------------
            if file_path_str.lower().endswith(".txt"):
                logger.info(f"Direct reading text file: {file_path_str}")
                try:
                    try:
                        with open(file_path_str, "r", encoding="utf-8") as f:
                            text_content = f.read()
                    except UnicodeDecodeError:
                        with open(file_path_str, "r", encoding="cp932") as f:
                            text_content = f.read()

                    processed = ExtractionOutput(title=title, content=text_content)
                    return {"extraction": processed}

                except Exception as e:
                    logger.error(f"Error reading txt file directly: {e}\n{traceback.format_exc()}")

            # ---------------------------------------------------------
            # 2. PDFファイルのバイパス処理 (NEW!)
            # ---------------------------------------------------------
            elif file_path_str.lower().endswith(".pdf"):
                logger.info(f"Direct reading PDF file (using pypdf): {file_path_str}")
                try:
        ...
        try:
            settings: ContentSettings = (
                await ContentSettings.get_instance()
            ...
            config_kwargs["docling_ocr"] = False
        ...
        try:
            ...
            logger.warning(
                f"Failed to retrieve speech-to-text model configuration: {e}"
            )
        ...
        if processed.title == "Error" and (
            processed.content or ""
        ).startswith("Failed to extract content:"):
            raise ValueError(
        ...
        if content_state.get("delete_source") and content_state.get(
            "file_path"
        ):
            ...
            try:
                ...
                logger.warning(
                    f"File not found while trying to delete: {file_path}"
                )
            except Exception as e:
        ...
        # ★ エラーの網：Tracebackをログに吐き出しつつDBのステータスをエラー状態に変更 ★
        error_trace = traceback.format_exc()
        logger.error(
            f"❌ [content_process Error] 処理中に例外が発生しました:\n{error_trace}"
        )

        # DBのSourceのステータス/タイトルをエラー状態に書き換えてグルグル待機を防止
        source_id = state.get("source_id")
        if source_id:
            try:
                source = await Source.get(source_id)
                if source:
                    source.title = f"Error: {str(e)[:50]}"
                    await source.save()
                    logger.info(
                        f"Source {source_id} のタイトルをエラー状態に更新しました。"
                    )
            except Exception as db_err:
                logger.error(
                    f"Sourceのステータス更新失敗: {db_err}\n{traceback.format_exc()}"
                )
...
async def save_source(state: SourceState) -> dict:
    try:
        ...
        # 変更後（安全な取得方法）
        extraction = state.get("extraction", None)
        if extraction is None:
            # extraction が取得できなかった場合のフォールバック処理
            logger.warning("state に 'extraction' が見つかりませんでした。スキップまたは初期値を設定します。")
            extraction = {} # または適切なデフォルト値
        ...
        if extraction.title and (
            not source.title or source.title == "Processing..."
        ):
        ...
        error_trace = traceback.format_exc()
        logger.error(
            f"❌ [save_source Error] 保存処理中に例外が発生しました:\n{error_trace}"
        )
        ...
        if source_id:
            try:
                ...
                logger.error(f"Sourceのステータス更新失敗: {db_err}")
